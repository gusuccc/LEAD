...
system_prompt = ""
with open('../instruction/amazon_itm.txt', 'r') as f:
    for line in f.readlines():
        system_prompt += line

# read the example prompts of items
example_prompts = []
with open('../data/amazon-CD/prompts_item.json', 'r') as f:
    for line in f.readlines():
...
# 创建一个文件来存储所有用户的返回结果
with open('../data/amazon-CD/profile_item.jsonl', 'w') as output_file:
    for idx, prompt in tqdm(enumerate(example_prompts), desc="Processing users"):
        ...
        output_file.write(json.dumps(item_response) + "\n")
